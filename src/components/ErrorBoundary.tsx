...
export class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render(): ReactNode {
      ...
      return (
        <div className="grid h-full min-w-0 place-items-center p-4">
          <div className="w-full min-w-0 max-w-md text-center">
            ...
            <pre className="mt-2 max-h-32 w-full overflow-auto whitespace-pre-wrap break-words rounded-md bg-rose-500/10 p-3 text-left text-[11px] text-rose-200">
              {error.message || String(error)}
            </pre>
            <button
              onClick={this.handleReset}
              className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-accent-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-accent-500"
            >
              Try again
            </button>
          </div>
    ...
    return (
      <div className="grid h-full min-h-screen min-w-0 place-items-center bg-ink-900 p-6 text-center">
        <div className="w-full min-w-0 max-w-lg">
          ...
          <pre className="mt-4 max-h-48 w-full overflow-auto whitespace-pre-wrap break-words rounded-md bg-rose-500/10 p-3 text-left text-xs text-rose-200">
            {error.message || String(error)}
          </pre>
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            <button
              onClick={this.handleReset}
              className="inline-flex min-h-[44px] items-center gap-1.5 rounded-md bg-accent-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-accent-500"
            >
              Try again
            </button>
            <button
              onClick={() => window.location.reload()}
              className="inline-flex min-h-[44px] items-center gap-1.5 rounded-md border border-ink-600 px-4 py-2 text-sm font-semibold text-ink-200 transition-colors hover:bg-ink-800"
            >
              Reload page
            </button>
          </div>
